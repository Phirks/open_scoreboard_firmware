...
char hex_cache_1 = 0x00;
char hex_cache_2 = 0x00;
...
{
	set_digit(0,timer_minutes/10,128,128,128);
	set_digit(1,timer_minutes%10,128,128,128);
	set_digit(2,timer_seconds/10,128,128,128);
	set_digit(3,timer_seconds%10,128,128,128);
	set_digit(4,0b00000000,0,0,0);
	set_digit(5,0b00000000,0,0,0);
	if (timer_seconds>99)
	{
...
void display_score()
{
	if (leftScore>199){leftScore=199;}
	if (leftScore<0){leftScore=0;}
	if (rightScore>199){rightScore=199;}
...
{
	second=second+1;
	if(second>59){minute=minute+1;second=0;}
...
{
	if(timer_minutes<1 && timer_seconds<1){k_timer_stop(&timer4);}
	if(timer_seconds<1){timer_minutes=timer_minutes-1;timer_seconds=59;}
	else{timer_seconds=timer_seconds-1;}
}
			  ...
			  uint16_t len)
{
	switch (data[0])
	{
		...
		case 0x07://displayHex
			...
			break;
		case 0x08://setTimer
			timer_minutes=data[1];
			timer_seconds=data[2];
			if (timer_seconds>59 && timer_minutes>0){
				timer_minutes=timer_minutes+timer_seconds/60;
	...
	for (;;) {
		...
		switch (mode){
			...
			case 0x01:
				...
				break;
			case 0x02:
				display_hex(hex_cache_1,hex_cache_2);
				break;
			case 0x03:
				display_raw();
				break;
